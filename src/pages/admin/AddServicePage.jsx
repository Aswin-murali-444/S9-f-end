...
import { Save, X, UploadCloud, Bell, Sun, Moon, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
const AddServicePage = () => {
  ...
  const checkServiceNameAvailability = async (name, categoryId) => {
    ...
    try {
      const result = await apiService.checkServiceNameAvailability(name, categoryId);
      if (!result.available) {
        setErrors(prev => ({
          ...prev,
          name: result.message
