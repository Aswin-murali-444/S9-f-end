...
const CustomerDashboard = () => {
  ...
  // Fetch team details for specific bookings
  const fetchTeamDetails = async (bookingId) => {
    try {
      const response = await apiService.getBookingTeamAssignments(bookingId);
  ...
  const generateMockTeamData = (booking) => {
    ...
    // Determine team size based on service type - Pest services use 2-person teams
    let teamSize = 2; // Default for pest control services
    let teamName = "Professional Pest Control Team";
    let teamDescription = "Specialized 2-person team for effective pest control";
    let teamLeader = "Rajesh Kumar";
    let teamPhone = "+91 98765 43210";
    let assignmentStatus = "assigned";
    
    // Adjust team size and details based on specific service
    if (booking.service_name?.toLowerCase().includes('termite')) {
      teamSize = 3; // Termite control needs more people
      teamName = "Expert Termite Control Squad";
      teamDescription = "Specialized 3-person team for comprehensive termite treatment";
      teamLeader = "Priya Sharma";
      teamPhone = "+91 98765 43211";
      assignmentStatus = "confirmed";
    } else if (booking.service_name?.toLowerCase().includes('fumigation')) {
      teamSize = 4; // Fumigation needs more people
      teamName = "Fumigation Specialists";
      teamDescription = "Certified 4-person team for safe fumigation services";
      teamLeader = "Amit Singh";
      teamPhone = "+91 98765 43212";
      assignmentStatus = "in_progress";
    }
    
    // Calculate team-based pricing
    const basePrice = booking.base_price || booking.total_amount || 1000;
    const calculatedPrice = basePrice * teamSize;
    
    // Ensure we have proper team assignment status
    if (!assignmentStatus) {
    ...
    return {
      ...
      team_leader_phone: teamPhone,
      assignment_status: assignmentStatus,
      // Calculate team-based pricing
      calculated_price: calculatedPrice,
      // Store original base price for display
  ...
  const isPestControlService = (serviceName, categoryName) => {
    ...
    // Exclude individual services that contain pest-related keywords but aren't team services
    const individualServices = ['mosquito net', 'mosquito nets', 'net', 'nets', 'repellent', 'repellents'];
    if (individualServices.some(service => serviceText.includes(service))) {
      return false;
    }
  ...
  const debugBookingData = (booking) => {
    console.log('Booking Debug Info:', {
      ...
      is_team_booking: booking.is_team_booking,
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (userError || !userData) {
        ...
        // Try booking_team_details view first, fallback to regular bookings
        ...
        // If view doesn't exist, fallback to regular bookings
        if (directError && (directError.message?.includes('relation "booking_team_details" does not exist') || directError.message?.includes('permission denied'))) {
          console.log('booking_team_details view not available, using regular bookings table');
        ...
        const transformedBookings = directData.map(booking => {
          const hasTeamData = booking.team_name !== undefined;
          const serviceName = hasTeamData ? (booking.service_name || 'Service') : (booking.services?.name || 'Service');
          const categoryName = hasTeamData ? (booking.service_category_name || 'General') : (booking.services?.service_categories?.name || 'General');
          
          // Check if this is a team service based on service name/category
          const isTeamService = isPestControlService(serviceName, categoryName);
          
          const baseBooking = {
            ...booking,
            service_name: serviceName,
            service_description: hasTeamData ? '' : (booking.services?.description || ''),
            user_name: user.user_metadata?.full_name || 'Customer',
            user_email: user.email || booking.contact_email,
            category_name: categoryName,
            // Team information
            is_team_booking: hasTeamData ? (booking.is_team_booking || false) : isTeamService,
            team_name: hasTeamData ? (booking.team_name || null) : null,
            team_description: hasTeamData ? (booking.team_description || null) : null,
            team_size_required: hasTeamData ? (booking.team_size_required || null) : null,
            team_leader_name: hasTeamData && booking.team_leader_first_name && booking.team_leader_last_name 
              ? `${booking.team_leader_first_name} ${booking.team_leader_last_name}` 
              : null,
            team_leader_phone: hasTeamData ? (booking.team_leader_phone || null) : null,
            assignment_status: hasTeamData ? (booking.assignment_status || null) : null,
            // Calculate team-based pricing if it's a team service
            calculated_price: hasTeamData && booking.is_team_booking && booking.team_size_required 
              ? (booking.base_price || booking.total_amount || 1000) * booking.team_size_required 
              : booking.total_amount || booking.base_price || 0
          };
          
          // Generate mock team data for team services
          return generateMockTeamData(baseBooking);
        });
      ...
      // Try to fetch bookings with team details using the view first
      ...
      // If view doesn't exist or fails, fallback to regular bookings table
      if (error && (error.message?.includes('relation "booking_team_details" does not exist') || error.message?.includes('permission denied'))) {
        console.log('booking_team_details view not available, using regular bookings table');
        const { data: fallbackData, error: fallbackError } = await supabase
          .from('bookings')
          .select(`
      ...
      // Transform the data to include service and user information with team details
      const transformedBookings = (data || []).map(booking => {
        // Check if this is team data (from booking_team_details view) or regular data
        const hasTeamData = booking.team_name !== undefined;
        const serviceName = hasTeamData ? (booking.service_name || 'Service') : (booking.services?.name || 'Service');
        const categoryName = hasTeamData ? (booking.service_category_name || 'General') : (booking.services?.service_categories?.name || 'General');
        ...
        const baseBooking = {
          ...
          user_email: user.email || booking.contact_email,
          category_name: categoryName,
          // Team information - only available if from booking_team_details view
          is_team_booking: hasTeamData ? (booking.is_team_booking || false) : isTeamService,
          team_name: hasTeamData ? (booking.team_name || null) : null,
          team_description: hasTeamData ? (booking.team_description || null) : null,
          team_size_required: hasTeamData ? (booking.team_size_required || null) : null,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Load categories and services in parallel with timeout
        ...
        const [categoriesData, servicesData] = await Promise.all([
          loadWithTimeout(apiService.getCategories()).catch(err => {
            console.warn('Categories API failed, using fallback:', err.message);
            return []; // Return empty array as fallback
          }),
          loadWithTimeout(apiService.getServices()).catch(err => {
            console.warn('Services API failed, using fallback:', err.message);
            return []; // Return empty array as fallback
          })
        ]);
        ...
        const mappedCategories = Array.isArray(categoriesData)
          ...
          : [
              // Fallback: Create a default "All Services" category even if API fails
              {
                id: '__all__',
                name: 'All Services',
                icon: AllServicesIcon,
        ...
        if (!isCancelled) {
          ...
          // Don't show error toast for API failures - just log and continue with empty data
          console.warn('Dashboard data loading failed, continuing with empty data:', errorMessage);
          ...
          // Only show error for critical failures, not API timeouts
          if (!errorMessage.includes('timeout') && !errorMessage.includes('fetch')) {
            setError({
              message: 'Some features may not be available. Please check your connection.',
              type: 'warning',
              details: {
    ...
    <div className="customer-dashboard">
      ...
      {!servicesLoading && categories.length === 1 && categories[0]?.services?.length === 0 && (
        <div style={{
          ...
          <h3 style={{ marginBottom: '0.5rem', color: '#1f2937' }}>Welcome to Your Dashboard!</h3>
          <p style={{ color: '#6b7280', marginBottom: '1rem' }}>
            Services are being loaded. If this message persists, please check your connection or contact support.
          </p>
          <button
                    ...
                    <div id="selected-category-services" className="services-by-category-section full-bleed">
                      ...
                      <div className="deals-grid">
                        {selectedCategory.services.map((service, idx) => {
                          ...
                          const discount = hasOffer && service.price ? Math.round(((service.price - service.offer_price) / service.price) * 100) : 0;
                          return (
                            <motion.div key={service.id} className="deal-card" whileHover={cardHoverEffects[idx % cardHoverEffects.length]} variants={cardEntranceVariants[idx % cardEntranceVariants.length]} onClick={() => handleServiceBooking(service)} style={{ cursor: 'pointer' }}>
                              {hasOffer && discount > 0 && (<div className="deal-badge">{discount}% OFF</div>)}
                              <div className="deal-image">
                                {service.icon_url ? (
                                  <img src={service.icon_url} alt={service.name} style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 8 }} onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextSibling.style.display = 'block'; }} />
                                      ...
                                      >
                                        ...
                                        {isTeamServiceForDisplay(service) && (
                                          <motion.div
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 1 }}
                                            transition={{ delay: idx * 0.1 + 0.55, duration: 0.4 }}
                                            style={{
                                              display: 'flex',
                                              alignItems: 'center',
                                              justifyContent: 'center',
                        ...
                        <div className="modal-body">
                          <div className="order-detail-section">
                            ...
                            <div className="detail-grid">
                              ...
                              {selectedOrder.is_team_booking && (
                                <>
                                  <div className="detail-item team-service">
                                    <span className="detail-label">
                                      <Users size={16} />
                                      Service Type:
                                    </span>
                                    <span className="detail-value team-badge" style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#0ea5e9', fontWeight: 600, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px', visibility: 'visible', opacity: 1 }}>Team Service</span>
                                  </div>
                                  {selectedOrder.team_name && (
                                    <div className="detail-item">
                                      <span className="detail-label">Team:</span>
                                      <span className="detail-value">{selectedOrder.team_name}</span>
                                    </div>
                                  )}
                                  {selectedOrder.team_description && (
                                    <div className="detail-item">
                                      <span className="detail-label">Team Description:</span>
                                      <span className="detail-value">{selectedOrder.team_description}</span>
                                    </div>
                                  )}
                                  {selectedOrder.team_size_required && (
                                    <div className="detail-item">
                                  ...
                                  {selectedOrder.team_leader_name && (
                                    <div className="detail-item">
                                      ...
                                      <span className="detail-value">{selectedOrder.team_leader_name}</span>
                                    </div>
                                  )}
                                  {selectedOrder.team_leader_phone && (
                                    <div className="detail-item">
                                      <span className="detail-label">Team Leader Phone:</span>
                                      <span className="detail-value">
                                        <a href={`tel:${selectedOrder.team_leader_phone}`} style={{ color: '#0ea5e9', textDecoration: 'none' }}>
                                          {selectedOrder.team_leader_phone}
                                        </a>
                                      </span>
                                    </div>
                                  )}
                                  {selectedOrder.assignment_status && (
                                    <div className="detail-item">
                                      <span className="detail-label">Assignment Status:</span>
                                      <span className="detail-value" style={{ 
                                        color: selectedOrder.assignment_status === 'confirmed' ? '#10b981' : 
                                               selectedOrder.assignment_status === 'in_progress' ? '#f59e0b' : '#6b7280',
                                        fontWeight: '600'
                                      }}>
                                        {selectedOrder.assignment_status}
                                      </span>
                                  ...
                                  {selectedOrder.calculated_price && selectedOrder.calculated_price !== selectedOrder.total_amount && (
                                    <div className="detail-item">
                                      <span className="detail-label">Team Pricing:</span>
                                      <span className="detail-value">
                                        ₹{selectedOrder.calculated_price.toFixed(2)} 
                                        <span style={{ fontSize: '0.875rem', color: '#6b7280', marginLeft: '8px' }}>
                                          (₹{(selectedOrder.original_base_price || selectedOrder.base_price || 1000).toFixed(2)} × {selectedOrder.team_size_required} members)
                                        </span>
                                      </span>
                                    </div>
                                  )}
                                ...
                                <div className="bill-details">
                                  ...
                                  {booking.is_team_booking && (
                                    <div className="team-info" style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '8px', padding: '8px 12px', background: 'linear-gradient(135deg, #f0f9ff, #e0f2fe)', border: '1px solid #0ea5e9', borderRadius: '8px', fontSize: '0.875rem', visibility: 'visible', opacity: 1, zIndex: 10, position: 'relative' }}>
                                      ...
                                      {booking.team_name && (
                                        <span className="team-name" style={{ color: '#0369a1', fontWeight: 600, fontSize: '0.875rem', visibility: 'visible', opacity: 1, display: 'block' }}>{booking.team_name}</span>
                                      )}
                                      ...
                                      {booking.assignment_status && (
                                        <span className="assignment-status" style={{ color: '#059669', fontWeight: 600, fontSize: '0.8rem', textTransform: 'capitalize', visibility: 'visible', opacity: 1, display: 'block' }}>Status: {booking.assignment_status}</span>
                                      )}
                                      {booking.calculated_price && booking.calculated_price !== booking.total_amount && (
                                        <div className="team-pricing-breakdown" style={{ background: 'linear-gradient(135deg, #f0fdf4, #dcfce7)', border: '1px solid #22c55e', borderRadius: '6px', padding: '6px 10px', marginTop: '4px', fontSize: '0.75rem', color: '#166534', visibility: 'visible', opacity: 1, display: 'block' }}>
                                          Team Pricing: ₹{booking.calculated_price.toFixed(2)} 
                                          (₹{(booking.original_base_price || booking.base_price || 1000).toFixed(2)} × {booking.team_size_required} members)
                                        </div>
                                      )}
                                    </div>
                                  )}
                      ...
                      <div className="payment-summary">
                        <div className="summary-stats">
                          <div className="stat-item">
                            ...
                            <div className="stat-content">
                              <h5>Total Spent</h5>
                              <p>₹{bookings.reduce((sum, booking) => sum + (booking.calculated_price || booking.total_amount || 0), 0).toFixed(2)}</p>
                            </div>
                          </div>
                          ...
                          <div className="stat-item">
                            ...
                            <div className="stat-content">
                              <h5>Team Services</h5>
                              <p>{bookings.filter(booking => booking.is_team_booking).length}</p>
                            </div>
                          </div>
                        </div>
                            ...
                            {bookings.slice(0, 3).map(booking => (
                              ...
                              >
                                ...
                                <div className="activity-content">
                                  ...
                                  <span>₹{(booking.calculated_price || booking.total_amount || 0).toFixed(2)} • {new Date(booking.scheduled_date).toLocaleDateString('en-IN')}</span>
                                </div>
